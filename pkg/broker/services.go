...
func (b *Broker) buildCatalog() error {
	...
	for _, providerName := range providerNames {
		...
		if providerName == "TENANT" {
			...
			u.Path = ""
			provider, err := atlasprivate.NewClient(u.String(), "", "", "").GetProvider(providerName)
			if err != nil {
				return err
			}
...
func (b *Broker) buildService(provider *atlasprivate.Provider) (service domain.Service) {
	...
	retrievable := b.mode == MultiGroup || b.mode == DynamicPlans
	...
	service = domain.Service{
		...
		Bindable:             true,
		InstancesRetrievable: retrievable,
		BindingsRetrievable:  false,
		Metadata:             nil,
		PlanUpdatable:        true,
...
func (b *Broker) buildPlansForProvider(provider *atlasprivate.Provider) []domain.ServicePlan {
	switch b.mode {
	case BasicAuth, MultiGroup:
...
func (*Broker) buildPlansForProviderStatic(provider *atlasprivate.Provider) []domain.ServicePlan {
...
func (b *Broker) buildPlansForProviderAuto(provider *atlasprivate.Provider) []domain.ServicePlan {
	var plans []domain.ServicePlan
...
func (b *Broker) buildPlansForProviderDynamic(provider *atlasprivate.Provider) []domain.ServicePlan {
	var plans []domain.ServicePlan
...
func planIDForInstanceSize(providerName string, instanceSize atlasprivate.InstanceSize, groupID string) string {
